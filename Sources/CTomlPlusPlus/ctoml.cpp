...
// Forward declaration
static CTomlNode convert_node(const toml::node &node,
                              struct CTomlTable *storage);
...
static CTomlNode convert_node(const toml::node &node, CTomlTable *storage) {
  ...
  if (node.is_string()) {
    ...
    result.data.string_value =
        storage->store_string(std::string(node.as_string()->get()));
    ...
    result.data.datetime_value.time.minute =
        static_cast<int32_t>(dt.time.minute);
    result.data.datetime_value.time.second =
        static_cast<int32_t>(dt.time.second);
    result.data.datetime_value.time.nanosecond =
        static_cast<int32_t>(dt.time.nanosecond);
    result.data.datetime_value.has_offset = dt.offset.has_value();
    result.data.datetime_value.offset_minutes =
        result.data.datetime_value.has_offset ? dt.offset->minutes : 0;
